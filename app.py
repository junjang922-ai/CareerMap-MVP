...
if st.session_state.step == 0:
    ...
    with col_h2:
        ...
        """, unsafe_allow_html=True)
    with col_h3:
        # 버튼을 감싸는 div에 flex를 적용하여 수직 중앙 정렬
        st.markdown("""
        <div style="display: flex; align-items: center; justify-content: flex-end; height: 42px;">
        """, unsafe_allow_html=True)
        # st.button은 직접 HTML 안에 넣을 수 없으므로, 위 div는 닫지 않고 CSS로 버튼 위치만 조정하는 방식 대신
        # Streamlit 버튼 자체에 margin-top을 주어 텍스트와 높이를 맞춤
        st.markdown("""
        <style>
    ...
    # 6. Bottom CTA
    st.markdown("""
    <div style="background-color:#F5F9FF; padding:50px; border-radius:20px; text-align:center; border:1px solid #E3F2FD;">
        <h2 style="margin:0 0 15px 0; color:#1A2B3C; font-size: 32px;">Ready to start your career in Korea?</h2>
        <p style="color:#546E7A; margin-bottom:30px; font-size: 18px;">Join 4,000+ international students managing their visa & career.</p>
    </div>
    """, unsafe_allow_html=True)
